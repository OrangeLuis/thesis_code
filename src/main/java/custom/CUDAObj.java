...
public class CUDAObj {
    ...
    public CUDA_MEMCPY2D cuTextureMemcpy(int[] nrb, CUdeviceptr nrbInputArray, int memoryDim) {
    ...
    public CUDA_MEMCPY2D cuTextureMemcpy(char[] nrb, CUdeviceptr nrbInputArray, int memoryDim) {
        CUDA_MEMCPY2D memcpy2D = new CUDA_MEMCPY2D();
        memcpy2D.srcMemoryType = CUmemorytype.CU_MEMORYTYPE_HOST;
        memcpy2D.srcHost = Pointer.to(nrb);
        memcpy2D.srcPitch = nrb.length;
    ...
    public CUdeviceptr createLinearTexture(int[] struct, int memoryDim) {
        ...
        CUDA_MEMCPY2D copy = cuTextureMemcpy(struct, pointer, memoryDim);
        cuMemcpy2D(copy);
